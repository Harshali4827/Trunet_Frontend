...
//   // Extract only damaged serials
//   const extractDamagedSerials = (serialNumbersArray) => {
...
//     if (!product.serialNumbers) return 0;
//     return product.serialNumbers.filter(sn => sn.status === "damaged").length;
//   };
...
//     const damagedSerials = extractDamagedSerials(product.serialNumbers);
...
//         // Only include serial numbers if product tracks them AND we have selected serials
//         if (row.selectedSerials && row.selectedSerials.length > 0) {
...
//     if (!product.serialNumbers) return { damaged: 0, underRepair: 0, repaired: 0, irreparable: 0 };
