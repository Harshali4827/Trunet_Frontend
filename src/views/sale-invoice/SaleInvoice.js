////***************** below is integrate with api only generate one time ************
...
// const markAsInvoiced = async (stockRequestIds, invoiceNumber) => {
//   try {
...
//     if (response.data.success) {
//       console.log('Marked as invoiced successfully:', response.data);
...
//       console.log('Marking as invoiced with:', {
//         stockRequestIds: selectedChallans,
//         invoiceNumber,
//         invoiceDate
//       });
  
//       try {
//         const invoicedResponse = await markAsInvoiced(selectedChallans, invoiceNumber);
        
//         if (invoicedResponse.success) {
//           console.log('Successfully marked as invoiced:', invoicedResponse.message);
//         } else {
//           throw new Error(invoicedResponse.message || 'Failed to mark as invoiced');
//         }
//       } catch (error) {
//         console.error('Error marking as invoiced:', error);
//         alert('Failed to mark stock requests as invoiced: ' + error.message);
//         return;
//       }
...
//       alert(`Invoice ${invoiceNumber} generated successfully for ${selectedChallans.length} stock request(s)!`);
...
//               // Check if invoice is already raised for this entry
...
// } from '@coreui/react';
// import CIcon from '@coreui/icons-react';
// import { cilArrowTop, cilArrowBottom, cilSearch, cilZoomOut, cilFile,cilCloudDownload } from '@coreui/icons';
// import { useNavigate } from 'react-router-dom';
// import { CFormLabel } from '@coreui/react-pro';
// import axiosInstance from 'src/axiosInstance';
// import Pagination from 'src/utils/Pagination';
...
//     const invoiceWindow = window.open('', '_blank');
//     if (invoiceWindow && invoice.invoiceHtml) {
//       invoiceWindow.document.write(invoice.invoiceHtml);
//       invoiceWindow.document.close();
//     } else {
//       alert('Invoice HTML not available');
//     }
//   };
...
//       // Prepare clean invoice data for storage
//       const invoiceData = {
...
//       if (saveSuccess) {
//         alert(`Invoice ${invoiceNumber} generated and saved successfully for ${selectedChallans.length} stock request(s)!`);
...
//                 </CTableDataCell>
//                 <CTableDataCell className="text-end">
//                   ₹{invoice.cgst?.toFixed(2) || '0.00'}
//                 </CTableDataCell>
//                 <CTableDataCell className="text-end">
//                   ₹{invoice.sgst?.toFixed(2) || '0.00'}
//                 </CTableDataCell>
//                 <CTableDataCell>
...
//                     placeholder={invoiceSearch.invoiceNumber || invoiceSearch.reseller ? "Disabled during advanced search" : "Search invoices..."}
//                   />
//                 </div>
//               </div>
...
const SaleInvoices = () => {
  ...
  const handleGenerateInvoice = async (metaData) => {
    ...
    try {
      ...
      // Generate the invoice HTML WITHOUT auto-print script for storage
      const invoiceHTML = `
        <html>
        <head>
          <title>Invoice - ${reseller?.businessName || 'SSV Alpha Broadband LLP'}</title>
