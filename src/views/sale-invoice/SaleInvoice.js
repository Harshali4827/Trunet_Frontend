...
// // Update the markAsInvoiced function to return the full response
// const markAsInvoiced = async (stockRequestIds, invoiceNumber) => {
//   try {
//     const response = await axiosInstance.post('/stockrequest/mark-invoiced', {
//       stockRequestIds,
//       invoiceNumber,
...
//     // Check if it's a validation error (already invoiced)
...
//       // FIRST: Mark stock requests as invoiced
//       try {
...
//         return; // Stop if marking as invoiced fails
//       }
...
//             <p><strong>Stock Requests Invoiced:</strong> ${selectedChallans.length}</p>
//             <p><strong>Total Amount:</strong> ₹ ${total.toFixed(2)}</p>
//             <p><em>This invoice has been saved in the system. You can now print it.</em></p>
...
//       // Update UI to reflect that entries are invoiced
//       // We need to refresh the data to get the updated invoiceInfo
//       fetchData(activeSearch, currentPage);
      
//       // Clear selected challans
//       setSelectedChallans([]);
...
const markAsInvoiced = async (stockRequestIds, invoiceNumber) => {
  ...
  const handleSelectAll = (e) => {
    // Only select entries that are not already invoiced
    if (e.target.checked) {
