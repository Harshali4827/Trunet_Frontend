...
const UsageDetail = () => {
  ...
  const handlePageChange = (page) => {
    if (page < 1 || page > totalPages) return;
    console.log('Page change to:', page, 'with activeSearch:', activeSearch);
    fetchData(activeSearch, page);
  };
  ...
  const getFlattenedData = () => {
    const flattened = [];
    const urlParams = getUrlParams();
    const requestedProductId = urlParams.product || activeSearch.product;
    
    console.log('Filtering for product ID:', requestedProductId);
    
    data.forEach(usage => {
      if (usage.items && usage.items.length > 0) {
        usage.items.forEach(item => {
          // Only include items that match the requested product
          if (!requestedProductId || item.product?._id === requestedProductId) {
            flattened.push({
              ...usage,
              item: item,
              product: item.product,
    ...
    console.log('Filtered data count:', flattened.length);
    return flattened;
  };
  ...
  const handleResetSearch = () => {
    ...
    // If we have URL parameters, preserve them
    if (urlParams.product && urlParams.center) {
      resetSearch = {
        ...resetSearch,
        product: urlParams.product,
  ...
  const generateDetailExport = async () => {
    try {
      ...
      } else if (urlParams.product && urlParams.center) {
        // Include URL filter info in filename
        fileName += `_${decodeURIComponent(urlParams.productName || 'Product').replace(/\s+/g, '_')}_at_${decodeURIComponent(urlParams.centerName || 'Center').replace(/\s+/g, '_')}`;
      }
