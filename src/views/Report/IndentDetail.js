...
{
const initialData = [
           {indentNo:'1111', date: '25 july 2025',center: 'Nashik',parentCenter:'Vashi warehouse', product: 'fiber' ,qty:'1',centerStock:'7',parentStock:'87',approvedQty:'9',},
           {indentNo:'4444', date: '25 july 2025',center: 'Nashik',parentCenter:'Vashi warehouse', product: 'Cat-5',qty:'2',centerStock:'67',parentStock:'98',approvedQty:'4',},
         ];
    ...
    return(
        <>
         {/* <h1 className='headingF'>Indent Detail Report</h1>  */}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
<tr>
  <th style={thStyle}>Indent No. {renderSortControls('indentNo')}</th>
  ...
  <th style={thStyle}>Center Stock {renderSortControls('centerStock')}</th>
  <th style={thStyle}>Parent Stock {renderSortControls('parentStock')}</th>
  <th style={thStyle}>Approved Qty. {renderSortControls('approvedQty')}</th>
</tr>
</thead>
<tbody>
  {data.map((item, index) => (
    <tr key={index}>
      <td style={tdStyle}>{item.indentNo}</td>
      <td style={tdStyle}>{item.date}</td>
      <td style={tdStyle}>{item.center}</td>
      <td style={tdStyle}>{item.parentCenter}</td>
      <td style={tdStyle}>{item.product}</td>
      <td style={tdStyle}>{item.qty}</td>
      <td style={tdStyle}>{item.centerStock}</td>
      <td style={tdStyle}>{item.parentStock}</td>
      <td style={tdStyle}>{item.approvedQty}</td>
    </tr>
                  ))}
                </tbody>
              </table>
